# Jupyter no Docker — ex. 11 sem ecrã (só localhost).
# Uso: .\run_jupyter.ps1
#      $env:EX11_NOTEBOOK='outro.ipynb'; .\run_jupyter.ps1
...
if (-not $env:EX11_NOTEBOOK) { $env:EX11_NOTEBOOK = 'exercicio_11_sem_ecra.ipynb' }
...
if (-not (Test-Path -LiteralPath $EnvFile)) {
    Write-Warning "Crie $EnvFile para a secção opcional com Gemini."
}

$nb = $env:EX11_NOTEBOOK.Replace(' ', '%20')
...
Write-Host ''
Write-Host '  Jupyter Lab — ex. 11 sem ecra (sem token)'
