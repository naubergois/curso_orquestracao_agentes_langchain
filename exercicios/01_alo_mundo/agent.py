...
def _modelo_configurado() -> str:
    # Predefinição mais tolerante a quota no plano gratuito; sobrescreva com GEMINI_MODEL no .env.
    nome = (os.environ.get("GEMINI_MODEL") or "gemini-1.5-flash").strip()
    return nome or "gemini-1.5-flash"
...
def run_chat_turn(turnos: list[dict[str, str]]) -> tuple[list[dict[str, str]], str]:
    ...
    if _eh_erro_limite_quota(ultimo_erro):
        ...
        raise RuntimeError(
            ...
            f"por exemplo no `.env`: GEMINI_MODEL=gemini-1.5-pro ou GEMINI_MODEL=gemini-2.0-flash "
            f"(agora: `{modelo}`). "
