...
#   Requer PostgreSQL acesível (compose expõe 5433 por defeito: POSTGRES_PORT).
...
run_local() {
  ...
  cd "${SCRIPT_DIR}"
  export DATABASE_URL="${DATABASE_URL:-postgresql://curso:curso@127.0.0.1:${POSTGRES_PORT:-5433}/pacientes}"
  echo "Local (venv): http://localhost:${STREAMLIT_PORT}  |  DATABASE_URL=${DATABASE_URL}"
...
run_docker() {
  ...
  [[ -f "${ENV_FILE}" ]] || echo "Aviso: crie ${ENV_FILE} com DEEPSEEK_API_KEY (ex. 4)." >&2
