# Jupyter no Docker — ex. 20 sem ecrã.
...
if (-not $env:EX20_NOTEBOOK) { $env:EX20_NOTEBOOK = 'exercicio_20_sem_ecra.ipynb' }
...
if (-not (Test-Path -LiteralPath $EnvFile)) {
    Write-Warning "Crie $EnvFile com GOOGLE_API_KEY (opcional para secção LLM)."
}

$nb = $env:EX20_NOTEBOOK.Replace(' ', '%20')
...
Write-Host ''
Write-Host '  Jupyter Lab — ex. 20 sem ecra (mercantil + paralelo + Pydantic; sem token)'
