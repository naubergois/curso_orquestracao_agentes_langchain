# Jupyter no Docker — ex. 16 sem ecrã.
...
if (-not $env:EX16_NOTEBOOK) { $env:EX16_NOTEBOOK = 'exercicio_16_sem_ecra.ipynb' }
...
if (-not (Test-Path -LiteralPath $EnvFile)) {
    Write-Warning "Crie $EnvFile com DEEPSEEK_API_KEY (ex. 16)."
}

$nb = $env:EX16_NOTEBOOK.Replace(' ', '%20')
...
Write-Host ''
Write-Host '  Jupyter Lab — ex. 16 sem ecra (crise voos simulada; sem token)'
